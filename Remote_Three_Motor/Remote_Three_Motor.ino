...
const int joyCpin = A2;		// centre joystick
...
const int motorCfwd = 2;	// centre motor forward pin
const int motorCbck = 4;	// centre motor backward pin
const int motorCen = 6;		// centre motor enable pin

// variables
int joyL;                   // left joystick reading (0-1023 from ADC)
int joyR;                   // right joystick reading (0-1023 from ADC)
int joyC;                   // centre joystick reading (0-1023 from ADC)
int joyLneutral;            // left joystick neutral position
int joyRneutral;            // right joystick neutral
int joyCneutral;            // centre joystick neutral
...
void loop() {  // code that loops forever
  ...
  Serial.println(motorRspeed);
   ...
   Serial.print("Centre: ");
  Serial.print(joyC);
  // set centre motor direction and speed
  if((joyC-joyCneutral) < -deadzone){ // joystick pushed forward
    digitalWrite(motorCfwd,HIGH);
    digitalWrite(motorCbck,LOW);
    motorCspeed = map(joyC,joyCneutral,0,0,255);
    Serial.print(" fwd ");
  }
  else if((joyC-joyCneutral) > deadzone){
    digitalWrite(motorCfwd,LOW);
    digitalWrite(motorCbck,HIGH);
    motorLspeed = map(joyC,joyCneutral,1023,0,255);
    Serial.print(" back ");
  }
  else{
    digitalWrite(motorCfwd,LOW);
  ...
  analogWrite(motorCen,motorCspeed);
